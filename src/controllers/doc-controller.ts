...
import {
  BasicFileFactory,
  IFileFactory,
} from "../classLibrary/engine/basicFileFactory";
...
export function DocController(core: Core, c: Command): void {
  c.command("build <src> <target>")
    ...
    .option(
      "-t, --tmp <tmp>",
      "path to tmp folder. default: random generated folder in /tmp",
    )
    .option(
    ...
    .action(async function (
      ...
      try {
        ...
        // Read configuration
        const configResult = await cfgReader
          .Read(configPath)
          ...
          .andThenAsync((ff) => artifactManager.Copy(ff)).promise;
    ...
    .action(async function (
      ...
      try {
        ...
        const configResult = await cfgReader
          ...
          .andThenAsync((config) =>
            generator
              .generateTemp(config, src)
              .map((v) => [v, config] as [string, InputConfig]),
          )
          .run(([dir]) => console.log(`Generated folder at ${dir}`))
          .andThenAsync(([dir, config]) =>
            exe
              .Version(dir, version)
              .map((v) => [v, config] as [IFileFactory, InputConfig]),
          )
          .andThenAsync(([ff, config]) => versioner.Copy(ff, config)).promise;
