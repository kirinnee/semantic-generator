...
class GitlintChecker {
  ...
  gitlintFile(rc: ReleaseConfiguration): PromiseResult<GitLintFile, string> {
    return this.fileFact
      .ReadSingle(rc.gitlint)
      .andThen((x) => ContentToString(x.content))
      .andThen((x) =>
        WrapAsError(
          "cannot find [contrib-title-conventional-commits]",
          x
            .LineBreak()
            .FindIndex((x) => x === "[contrib-title-conventional-commits]"),
        )
          .andThen((index) =>
            WrapAsError("no line after [contrib-title-conventional-commits]", [
              x.LineBreak()[index + 1],
              index,
            ] as [string, number]),
          )
  ...
  Check(rc: ReleaseConfiguration): PromiseResult<string, string> {
    ...
    return this.gitlintFile(rc)
      ...
      .andThenAsync(async (types): Promise<Result<string, string>> => {
        ...
        if (this.core.DeepEqual(rcTypes, types))
          return Ok(".gitlint properly configured");
        return this.write(rc).andThen(
          (): Result<string, string> =>
            Err(`gitlint does not match: ${rcTypes}`),
        ).promise;
      });
  }
  ...
  ): PromiseResult<IWritable, string> {
    const rcTypes = rc.types.Map((x) => x.type).Sort(SortType.AtoZ);
    return this.gitlintFile(rc)
      .map((gitlint) =>
        gitlint.content.ReplaceAll(
          gitlint.keyLine,
          `types = ${rcTypes.join(",")}`,
          false,
        ),
  ...
  write(rc: ReleaseConfiguration): PromiseResult<string, string> {
    return this.generateGitLintFile(rc).andThenAsync(async (x) => {
      const r = await this.writer.Write(x);
      return r.match({
        none: (): Result<string, string> =>
          Ok("successfully updated gitlint configuration"),
        some: (s) => Err(s),
      });
    });
  }
