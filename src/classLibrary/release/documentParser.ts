...
class CommitConventionDocumentParser {
  ...
  generateVaeDocs(rc: ReleaseConfiguration, t: string): Result<string, string> {
    return WrapAsError(
      `cannot find type entry: ${t}`,
      rc.types.Find((x) => x.type == t),
    ).andThen((x) =>
      Wrap(x.vae).match({
        none: () => Ok(""),
        some: (s: Vae) => {
          ...
          return this.mdt.Render([
            ...
            [
              "application",
              `when this commit is applied, it will _${s.verb}_ \`${s.application}\``,
            ],
            ["example", s.example],
            [
              "example applied",
              `when this commit is applied, it will _${s.verb}_ ${a}`,
            ],
          ]);
        },
      }),
