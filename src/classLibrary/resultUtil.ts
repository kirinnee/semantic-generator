...
class PromiseResult<V, E> {
  public static Invert<T, Err>(
    i: Result<Promise<T>, Err>,
  ): PromiseResult<T, Err> {
    return PR(async (): Promise<Result<T, Err>> => {
      if (i.isOk()) {
  ...
  constructor(result: Promise<Result<V, E>> | Result<V, E>) {
    if (
      (<Promise<Result<V, E>>>result).then &&
      (<Promise<Result<V, E>>>result).catch &&
      (<Promise<Result<V, E>>>result).finally
    ) {
  ...
  andThenAsync<X>(
    func: (v: V) => Promise<Result<X, E>> | PromiseResult<X, E>,
  ): PromiseResult<X, E> {
    return PR(async (): Promise<Result<X, E>> => {
