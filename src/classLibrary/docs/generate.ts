...
class Generator {
  ...
  resolveStatic(config: InputConfig): PromiseResult<IWritable[], string[]> {
    ...
    const meta = subFact
      .Scan("**/*")
      .Each((x) => (x.original = path.join("static", x.original)));
    return subFact
      .Read(meta)
  ...
  resolveDocs(config: InputConfig): PromiseResult<IWritable[], string[]> {
    const f = config.navbar.tabs
      .Map((x) => this.sourceFileFactory.Scan(`${x.folder}/**/*`))
      .Flatten<FileMetadata>();
    return this.sourceFileFactory
      .Read(f)
      .map((files) =>
        files
  ...
  resolveMeta(
    ...
    if (meta.landing.button)
      meta.landing.button.to = path.join(sourceFolder, meta.landing.button.to);
  ...
  resolveSideBar(config: InputConfig): string {
    return config.navbar.tabs
      .Map(
        (x) =>
          `sidebar${x.name
            .split("")
            .Where((x) => x.IsAlphanumeric())
            .join("")}: [{type: 'autogenerated', dirName: '${x.folder}' }],`,
      )
      .join("\n");
  }
  ...
  resolveFeatures(
    feature: Feature[],
  ): PromiseResult<[IWritable[], string], string[]> {
    return PromiseResultAll(
      feature.Map(({ svg, desc, title }, idx) =>
        this.sourceFileFactory.ReadSingle(svg).map((svgFile) => {
          ...
          new Promise<Result<[IWritable[], string[]], string>>((r) => {
            favicons(
              ...
              function (err, resp) {
                ...
                // copy favicon
                const favico: Option<IWritable> = Wrap(
                  resp.images.Find((f) => f.name === "favicon.ico"),
                ).map((f) => {
  ...
  resolveTemplates(
    ...
    const resolvedMeta = this.resolvers.reduce(
      (a, b) => b.ResolveMeta(vals, a),
      result,
    );
    return this.templateFileFactory
      ...
      .map((files) =>
        this.resolvers
          .reduce(
            (file, resolver) => resolver.ResolveContent(vals, file),
            files,
          )
  ...
  headToHeaderNode(i: string[]): HeadNode[] {
    return i
      .Map(
        (html) =>
          parse(html).firstChild as unknown as {
            rawTagName: string;
    ...
    return PromiseResultTupleAll(
      ...
      .map(
        ([
          ...
          docs,
          [faviconFiles, header],
          [featureFile, featureCode],
        ]) =>
          [
            [...docs, ...faviconFiles, ...featureFile, ...statics, ...versions],
            header,
            featureCode,
          ] as [files: IWritable[], elements: string[], code: string],
      )
      .map(
        ([files, elements, code]) =>
          [
            [
              this.resolveMeta(
                config,
                this.headToHeaderNode(elements),
                targetFolder,
              ),
              ...files,
            ],
            code,
          ] as [files: IWritable[], code: string],
      )
      .andThenAsync(([files, code]) =>
        this.resolveTemplates(
          theme,
          config.landing?.enable ?? false,
          sidebar,
          code,
        ).map((w) => [...w, ...files]),
      )
  ...
  constructor(
    templateFileFactory: IFileFactory,
    sourceFileFactory: IFileFactory,
    tmpWriter: Writer,
    resolvers: Resolver[],
  ) {
