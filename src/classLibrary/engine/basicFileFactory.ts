...
function ReadBinary(path: string): PromiseResult<Buffer, string> {
  return new PromiseResult<Buffer, string>(
    new Promise<Result<Buffer, string>>((r) => {
...
function ReadFile(path: string): PromiseResult<Content, string> {
  return new PromiseResult<Content, string>(
    new Promise<Result<Content, string>>((r) => {
      fs.readFile(path, function (err, data) {
        if (err) {
          r(Err(err.message));
          return;
...
class BasicFileFactory implements IFileFactory {
  ...
  ReadSingle(p: string): PromiseResult<VFile, string> {
    ...
    return ReadFile(readPath).map((c) => {
      return {
        meta: {
          original: p,
          from: readPath,
  ...
  Read(files: FileMetadata[]): PromiseResult<VFile[], string[]> {
    return PromiseResultAll(
      files.Map((meta) =>
        ReadFile(meta.from).map((content) => {
          return {
            content,
            meta,
          };
