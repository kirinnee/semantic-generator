import {
  ...
  IWritable,
  Writer,
} from "../../src/classLibrary/engine/writer";
...
describe("BasicWriter", function () {
  ...
  describe("BatchWrite", function () {
    it("should write File objects", async function () {
      ...
      const result = await writer.BatchWrite([
        IWritable.File(subj1),
        IWritable.File(subj2),
      ]);
      expect(result.isNone()).toBe(true);
      {
        // sample1 assert
        const r = await ReadBinary("test_ground/write_test/sample1.txt")
          .promise;
      ...
      {
        // sample2 assert
        const r = await ReadBinary("test_ground/write_test/sample2.txt")
          .promise;
        expect(r.isOk()).toBe(true);
        const act = r.unwrap().toString("utf8");
    ...
    it("should write FaviconImages", async function () {
      ...
      const result = await writer.BatchWrite([
        IWritable.FaviconImage(subj1),
        IWritable.FaviconImage(subj2),
      ]);
      expect(result.isNone()).toBe(true);
      {
        // sample1 assert
        const r = await ReadBinary("test_ground/write_test/sample4/sample3.txt")
          .promise;
        expect(r.isOk()).toBe(true);
        const act = r.unwrap().toString("utf8");
      ...
      {
        // sample2 assert
        const r = await ReadBinary(
          "test_ground/write_test/sample/a/b/c/d/sample4.txt",
        ).promise;
        expect(r.isOk()).toBe(true);
        const act = r.unwrap().toString("utf8");
    ...
    it("should write FaviconFiles", async function () {
      ...
      const result = await writer.BatchWrite([
        IWritable.FaviconFile(subj1),
        IWritable.FaviconFile(subj2),
      ]);
      expect(result.isNone()).toBe(true);
      {
        // sample1 assert
        const r = await ReadBinary(
          "test_ground/write_test/path/next/sample10.txt",
        ).promise;
        expect(r.isOk()).toBe(true);
        const act = r.unwrap().toString("utf8");
      ...
      }
      {
        // sample2 assert
        const r = await ReadBinary(
          "test_ground/write_test/path/next/more/nest/sample11.txt",
        ).promise;
        expect(r.isOk()).toBe(true);
        const act = r.unwrap().toString("utf8");
    ...
    it("should fail if file is already a directory", async function () {
      ...
      const writable = IWritable.FaviconFile({
        name: "tmp",
        contents: "hello",
      });
